...
export class Template {
	...
	private replacePlaceholders() {
		this.rendered = this.html.replace(
			/\$\{\s?(\w+)\s?\|?\s?(\w*)\s?\}/g,
			(all, name, option) => {
	...
	private injectHTML(html: string, prepend = false) {
		if (prepend) {
